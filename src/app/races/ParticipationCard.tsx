...
export default function ParticipationCard({ raceId }: ParticipationCardProps) {
  ...
  const toggle = async (field: string, val: boolean) => {
    if (!user) { alert("Zaloguj siê!"); return; }
    ...
    if (error) {
      ...
      // alert("B³¹d zapisu! SprawdŸ SQL w Supabase."); 
    }
  };
  ...
  return (
    <section style={{ background: "rgba(255,255,255,0.05)", padding: 30, borderRadius: 24, border: "1px solid #333" }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 15, marginTop: 20 }}>
        <label style={lab}><input type="checkbox" checked={status.is_cheering} onChange={e => toggle("is_cheering", e.target.checked)} style={chk} /> <span style={{color: status.is_cheering ? "#ffff00" : "#888"}}>1. CHCÊ POBIEC</span></label>
