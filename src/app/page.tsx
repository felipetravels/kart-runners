  ...
  return (
    <div style={{ 
      ...
      <header style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "40px 0", height: "125px" }}>
        ...
        <nav style={{ display: "flex", gap: "30px", fontWeight: 900, textTransform: "uppercase" }}>
          ...
          <div style={{ border: "2px solid #00d4ff", borderRadius: "15px", padding: "10px 30px", background: "rgba(0,0,0,0.6)" }}>
            <span style={{ fontWeight: 900, color: "#fff" }}>Filip</span>
          </div>
        </nav>
      </header>

      <main style={{ maxWidth: "1200px", margin: "0 auto", paddingTop: "80px" }}>
        <section style={{ display: "flex", gap: "60px", marginBottom: "80px" }}>
          <div style={{ flex: 1 }}>
            <p style={labelS}>WSPÓLNE KILOMETRY</p>
            <h2 style={{ fontSize: "6.5rem", fontWeight: 900, color: "#00d4ff", margin: 0 }}>{totalKm.toFixed(1)} km</h2>
          </div>
          <div style={{ flex: 1 }}>
        ...
        <section style={{ marginBottom: "80px" }}>
          <p style={labelS}>REKORDY DYSTANSÓW (TOP 3)</p>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "20px" }}>
            {["5K", "10K", "21K"].map(dist => (
              <div key={dist} style={topBoxS}>
                <h4 style={{ color: "#00d4ff", margin: "0 0 15px 0" }}>{dist}</h4>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(380px, 1fr))", gap: "30px" }}>
          {futureRaces.map(r => (
            <div key={r.id} style={cardS}>
              ...
              <h3 style={{ fontSize: "2rem", margin: "10px 0" }}>{r.title}</h3>
              <div style={{ borderTop: "1px solid #333", paddingTop: "15px" }}>
                <p style={{ fontSize: "0.8rem", color: "#888", marginBottom: "10px" }}>OPŁACILI START:</p>
                {r.paidParticipants.map((p: any, i: number) => (
                  <div key={i} style={{ display: "flex", justifyContent: "space-between", marginBottom: "5px" }}>
                    <span>{p.display_name}</span>
                    <span style={{ color: "#00d4ff", fontWeight: 900, fontSize: "0.7rem" }}>OPŁACONE</span>
                  </div>
                ))}
                {r.paidParticipants.length === 0 && <span style={{ color: "#444" }}>Brak opłaconych startów</span>}
              </div>
            </div>
          ))}
        </div>
...
const labelS = { color: "#444", fontWeight: 900, letterSpacing: "2px", marginBottom: "15px" };
const topBoxS = { background: "rgba(255,255,255,0.03)", padding: "25px", borderRadius: "20px", border: "1px solid #1a1a1a" };
const cardS = { background: "rgba(255,255,255,0.04)", padding: "40px", borderRadius: "30px", border: "1px solid #1a1a1a", backdropFilter: "blur(10px)" };
const addBtnS = { background: "#00d4ff", color: "#000", padding: "15px 40px", borderRadius: "15px", fontWeight: 900, textDecoration: "none" };