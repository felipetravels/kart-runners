...
export default function HomePage() {
  ...
  useEffect(() => {
    async function fetchAll() {
      ...
      const [r, k, t, p, res] = await Promise.all([
        ...
        supabase.from("v_participation_details").select("*").eq("is_paid", true),
    ...
    <main style={{ maxWidth: "1400px", margin: "0 auto", padding: "40px 20px", color: "#fff" }}>
      ...
      <div style={grid}>
        {upcoming.map(r => {
          ...
          return (
            <div key={r.id} style={raceContainer}>
              <RaceCard race={r} />
              
              {/* LISTA STARTOWA POD KARTĄ */}
              <div style={participationBox}>
                ...
                <div style={{ display: "flex", flexWrap: "wrap", gap: "8px" }}>
                  {racePaid.map(p => {
                    ...
                    return (
                      <span key={p.user_id} style={hasFinished ? finishedBadge : waitingBadge}>
                        {hasFinished && "🏅 "}{p.display_name}
                      </span>
                    );
                  })}
                  {racePaid.length === 0 && <span style={{fontSize: "0.8rem", opacity: 0.3}}>Nikt jeszcze nie opłacił.</span>}
                </div>
              </div>
            </div>
          );
      ...
      <h2 style={{...secH, marginTop: 80, opacity: 0.5}}>ARCHIWUM BIEGÓW</h2>
      <div style={grid}>
        {past.map(r => (
...
const raceContainer = { display: "flex", flexDirection: "column" as const, gap: "10px" };
const participationBox = { background: "rgba(255,255,255,0.03)", padding: "15px", borderRadius: "15px", border: "1px solid #222" };
const countNum = { fontSize: "1.8rem", fontWeight: 900, color: "#00d4ff", lineHeight: 1 };
