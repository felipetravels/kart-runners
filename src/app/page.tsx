  ...
  return (
    <main style={{ maxWidth: "1400px", margin: "0 auto", padding: "40px 20px", color: "#fff" }}>
      ...
      <div style={{ marginTop: 100, borderTop: "1px solid #333", paddingTop: 40, textAlign: "center" }}>
        <button onClick={() => setShowAdmin(!showAdmin)} style={{ background: "transparent", border: "1px solid #444", color: "#888", padding: "10px 20px", borderRadius: 5, cursor: "pointer", fontSize: "0.8rem" }}>
        ...
        {showAdmin && (
          <div style={{ marginTop: 30, background: "rgba(255,255,255,0.05)", padding: 30, borderRadius: 15, display: "inline-block", textAlign: "left" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 15, width: "300px" }}>
              <input placeholder="Nazwa Biegu" value={newRace.title} onChange={e => setNewRace({...newRace, title: e.target.value})} style={inputStyle} />
              <input type="date" value={newRace.date} onChange={e => setNewRace({...newRace, date: e.target.value})} style={inputStyle} />
              <input placeholder="Dystans (km)" type="number" value={newRace.distance} onChange={e => setNewRace({...newRace, distance: e.target.value})} style={inputStyle} />
              <select value={newRace.type} onChange={e => setNewRace({...newRace, type: e.target.value})} style={inputStyle}>
                <option value="road">Ulica</option>
                <option value="trail">Trail</option>
                <option value="track">Bieżnia</option>
              </select>
              <button onClick={handleAddRace} style={{ background: "#00d4ff", color: "#000", border: "none", padding: "12px", borderRadius: 5, fontWeight: "bold", cursor: "pointer", marginTop: 10 }}>
